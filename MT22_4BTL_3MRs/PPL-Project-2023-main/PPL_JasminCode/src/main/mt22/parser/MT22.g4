...
full_var_decl:
	ID COMMA full_var_decl COMMA expr
	| ID COLON var_type ASS expr;

func_decl:
	ID COLON FUNCTION func_ret_type param_list (INHERIT ID)? block_stmt;
...
match_stmt:
	IF sub_expr match_stmt ELSE match_stmt
	| assign_stmt
	| call_stmt
	| return_stmt
	| for_stmt
...
constant:
	...
	| array_lit;
sub_expr: LR expr RR;
index_operator: LS exprprime RS; // [3 + foo(2)]
...
lhs: ID | index_expr;
index_expr: ID index_operator; // a[3 + foo(2)]
...
STRINGLIT:
	["] STRINGABLE_CHARACTER* ["] {self.text = self.text[1:-1]};
...
UNCLOSE_STRING:
	'"' STRINGABLE_CHARACTER* (EOF | [\r\n]) {
        raise UncloseString(self.text[1:].replace('\n','').replace('\r',''))
};

ILLEGAL_ESCAPE:
	["] STRINGABLE_CHARACTER* ILLEGAL_ESCAPE_CHARACTER {
